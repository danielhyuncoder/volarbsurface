...
def calculate_matrix(x_labels: np.array, y_labels: np.array, y_axis: str, realized_volatility: float, K:float, T:float, r:float, option_price: float, option_type:str):
    ...
    for j in y_labels:
        ...
        for i in x_labels:
            if "Risk" in y_axis:
               row.append(np.round(realized_volatility-implied_volatility(i, K, T, j, option_price, option_type=option_type), 2))

            else:
               row.append(np.round(realized_volatility-implied_volatility(i,K,i,r,option_price, option_type=option_type), 2))
        matrix.append(row)
